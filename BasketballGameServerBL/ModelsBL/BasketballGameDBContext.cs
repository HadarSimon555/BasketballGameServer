...
namespace BasketballGameServerBL.Models
{
    public partial class BasketballGameDBContext : DbContext
    {
        ...
        {
            return this.Users.Include(u => u.Players).ThenInclude(t=>t.RequestToJoinTeams).Include(u=>u.Players).ThenInclude(u => u.Team).Include(u => u.Coaches).ThenInclude(u => u.Team.Players).ThenInclude(c => c.RequestToJoinTeams).Where(u => u.Email == email && u.Pass == pass).FirstOrDefault();
        }
        #endregion
